...
fn copy_running_state_to_stream(
    ...
    let keys = CuDFTable::concat_on_stream(vec![key_view.clone()], stream).map_err(cudf_to_df)?;
    let state_columns = state_views
        .iter()
        .map(|view| CuDFColumn::concat_on_stream(vec![view.clone()], stream).map_err(cudf_to_df))
        .collect::<Result<Vec<_>>>()?;

    // cuDF copies asynchronously on `stream`. Keep the source views, and thus
    // their buffers, alive until those copies complete.
    stream.synchronize().map_err(cudf_to_df)?;
...
/// ```text
///                              Running
///                  route each batch by CUDA stream
///                   /             |             \
///                  v              v              v
///             lane 0         lane 1    ...   lane N
///          pending buffer  pending buffer    pending buffer
///                  |              |              |
///          chunk-sized or EOF: spawn one blocking task per ready lane
///                  |              |              |
///          update lane state update lane state  update lane state
///                  |              |              |
///          repeat while polling input; buffer the next chunk while a task runs
///                   \             |             /
///                    EOF: flush buffers and await every lane task
///                                  |
///                                  v
///                             Finalizing
///             synchronize lanes and choose a primary stream
///            copy secondary states onto it, merge, build output
///                                  |
///                                  v
///                                Done
/// ```
enum StreamState {
    ...
    Running,
    /// Reconcile lane states and construct the single output batch.
