...
namespace libcudf_bridge {
    ...
    [[nodiscard]] std::unique_ptr<DeviceAsyncResourceRef> set_current_device_resource_ref(
        const DeviceAsyncResourceRef& resource);

    /// Reset cuDF's current device memory resource reference to the initial resource.
    [[nodiscard]] std::unique_ptr<DeviceAsyncResourceRef> reset_current_device_resource_ref();
